...
fn echo_builtin_round_trips_through_typed_mint() {
    ...
    let output = slot
        .invoke(Rights::INVOKE, input.clone())
...
fn plugin_cspaces_are_isolated_until_explicit_transfer() {
    ...
    let out = typed_slot
        .invoke(Rights::INVOKE, serde_json::json!({"hi": "agent"}))
...
fn agent_manifest_requires_resolve_to_a_binding_row_for_every_handle() {
    ...
    use odyssey_builtin::{agent, database, echo, reverse, streaming_echo};
...
fn agent_reaches_only_its_bindings_and_reports_revocation() {
    ...
    use odyssey_builtin::agent::{AgentCore, AgentError};
    ...
    assert_eq!(described["operations"].as_array().map(Vec::len), Some(2));
...
fn agent_describe_refuses_unknown_fields() {
    ...
    use odyssey::core::manifest::manifest::CapabilityDecl;
    use odyssey_builtin::agent::AgentDescribeResource;
...
fn frontend_script_binds_to_the_live_api() {
    ...
    let bin: PathBuf = match option_env!("CARGO_BIN_EXE_odyssey-example-back") {
        ...
        // env var isn't baked into the test's rmeta (e.g. a
        // developer running `cargo test` from outside the example
        // workspace). The layout is the example workspace's own
        // `target/`, not the root.
        None => {
            let mut dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
            dir.push("target");
            dir.push(std::env::var("PROFILE").unwrap_or_else(|_| "debug".into()));
            dir.push("odyssey-example-basic");
            dir
        }
    };
    assert!(
        bin.exists(),
        "the example binary is missing at {} — run `cargo build` in example/ first",
        bin.display()
    );
...
fn llm_complete_builtin_round_trips_through_typed_mint() {
    ...
    use odyssey_builtin::llm::{LlmBuiltin, LlmCompleteResource};
...
fn llm_embed_builtin_round_trips_through_typed_mint() {
    ...
    use odyssey_builtin::llm::{LlmBuiltin, LlmEmbedResource};
...
fn memory_query_finds_inserted_record_by_substring() {
    use odyssey_builtin::memory::{MemoryBuiltin, MemoryInsertResource, MemoryQueryResource};
...
fn tool_descriptor_returns_schema_for_every_tool_builtin() {
    ...
    use odyssey_builtin::streaming_echo::StreamingEchoBuiltin;
    use odyssey_builtin::tool_descriptor::{ToolDescriptorBuiltin, ToolDescriptorResource};
    ...
    for tool in ["echo", "reverse", "database", "streaming_echo"] {
        let out = slot
            .invoke(Rights::INVOKE, serde_json::json!({"tool": tool}))
...
fn profile_inspector_returns_cap_meta_and_operations() {
    use odyssey_builtin::profile_inspector::{ProfileInspectorBuiltin, ProfileInspectorResource};
    ...
    assert!(
        out["operations"].as_array().unwrap().len() == 2,
...
fn agent_runtime_manifest_resolves_to_four_bindings() {
    use odyssey::personality::composition::resolve::resolve;
    use odyssey_builtin::{agent_runtime, llm, memory, profile_inspector, tool_descriptor};
...
fn agent_runtime_session_lifecycle_with_mock_llm() {
    ...
    use odyssey_builtin::{llm, memory};
    ...
    match step1 {
        odyssey_builtin::agent_runtime::Step::ToolResult(ref r) => {
    ...
    assert!(
        matches!(step2, odyssey_builtin::agent_runtime::Step::Final(_)),
...
fn agent_plan_returns_text_step() {
    ...
    use odyssey::core::Resource;
    use odyssey::personality::composition::resolve::resolve;
    use odyssey_builtin::agent_runtime::{AgentPlanResource, AgentRuntimeBuiltin};
    use odyssey_builtin::{llm, memory};
...
fn openai_backend_new_stores_config() {
    use odyssey_builtin::llm::OpenAiBackend;
...
fn backend_from_env_picks_real_or_mock() {
    use odyssey_builtin::llm::{CompleteRequest, backend_from_env};
    let _guard = env_lock().lock().unwrap_or_else(|e| e.into_inner());
...
async fn openai_backend_round_trips_through_local_http_server() {
    ...
    use odyssey_builtin::llm::{LlmBackend, OpenAiBackend, backend_from_env};
    ...
    let resp = openai
        .complete(odyssey_builtin::llm::CompleteRequest {
            prompt: "say 42".into(),
...
async fn openai_backend_parses_native_tool_calls() {
    use axum::{Router, extract::Json as AxJson, response::Json, routing::post};
    use odyssey_builtin::llm::{CompleteRequest, LlmBackend, OpenAiBackend, ToolDefinition};
    use serde_json::Value;
    use std::sync::{Arc, Mutex};
...
fn llm_streaming_deltas_reach_session_broadcast() {
    ...
    let manifests = vec![
        odyssey_builtin::llm::LlmBuiltin.manifest(),
        odyssey_builtin::memory::MemoryBuiltin.manifest(),
        AgentRuntimeBuiltin.manifest(),
    ];
    let plan = resolve(&manifests).expect("resolve must succeed");
    let cspace = CapabilitySpace::new();
...
fn file_memory_backend_persists_records_across_reopen() {
    ...
    use odyssey_builtin::memory::{FileMemoryBackend, MemoryInsertResource, Record};
...
fn agent_session_can_be_paused_and_loaded() {
    ...
    let (step, _history, _status) = runtime
        .advance(
            sid.as_str(),
            odyssey_builtin::agent_runtime::Observation::Tick,
        )
        .expect("advance");
    assert!(
        matches!(step, odyssey_builtin::agent_runtime::Step::ToolResult(_)),
