//! Echo builtin — streaming pass-through that emits `count`
...
//! path, the `/api/stream` SSE bridge) is exercised here.
//!
...
/// Echo resource — `open` emits `count` chunks then `Done`
...
impl Resource for EchoResource {
    fn open(&self, input: Value) -> Result<mpsc::Receiver<CapabilityChunk>, String> {
        let text = input
            .get("text")
            .and_then(|v| v.as_str())
            .ok_or_else(|| format!("echo: expected {{\"text\": \"<string>\"}}, got {}", input))?
            .to_string();
        let count = input
            .get("count")
            .and_then(|v| v.as_u64())
            .ok_or_else(|| format!("echo: expected {{\"count\": <u64>}}, got {}", input))?;
...
impl BuiltinManifest for EchoBuiltin {
    ...
    fn manifest(&self) -> PluginManifest {
        ...
        ManifestBuilder::new("echo")
            .expose_streaming_with_schema("echo", "echo", tool_schema)
            .timeout_ms(5000)
            .build()
    }
}
...
impl EchoBuiltin {
    ...
    /// lives in the plugin's own `PluginCspace`. We then grant
    /// a derived slot into the orchestrator's global cspace
    /// so the HTTP bridge can still look it up by name. The
    /// returned `SlotId` is the GLOBAL one — the
