...
impl OpenAiBackend {
    ...
    fn http_post(&self, endpoint: &str, body: &Value) -> Result<String, String> {
        ...
        let body_str =
            serde_json::to_string(body).map_err(|e| format!("openai: serialise body: {e}"))?;
...
impl LlmBackend for OpenAiBackend {
    ...
    fn embed(&self, texts: &[String]) -> Result<Vec<Vec<f32>>, String> {
        ...
        let v: Value =
            serde_json::from_str(&text).map_err(|e| format!("openai embed: parse JSON: {e}"))?;
        let data = v
            .get("data")
            .and_then(Value::as_array)
                ...
                arr.iter()
                    .map(|t| {
                        let name = t
                            .get("name")
                            .and_then(Value::as_str)
                            .ok_or_else(|| "llm_complete: tool entry missing `name`".to_string())?
                            .to_string();
                        let description = t
                            .get("description")
                            .and_then(Value::as_str)
