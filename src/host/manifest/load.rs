...
//!
//! Phase 5 D5: the loader explicitly detects the deleted quota
//! fields (`tokens_per_minute`, `bytes_per_minute`) and emits a
//! warning so operators on upgraded manifests see a diagnostic
//! instead of a silent field drop. The fields are then ignored
...
//! deprecated keys, not enforce their values. False positives
...
/// Scan the TOML source for the deleted quota fields and emit
...
fn warn_deprecated_quota_fields(s: &str) {
    if s.contains("tokens_per_minute") {
        eprintln!(
            "[manifest] WARN: source contains the deleted `tokens_per_minute` quota \
             field; Phase 5 removed it from QuotaSpec — limit is dropped"
        );
    }
    if s.contains("bytes_per_minute") {
        eprintln!(
            "[manifest] WARN: source contains the deleted `bytes_per_minute` quota \
             field; Phase 5 removed it from QuotaSpec — limit is dropped"
        );
    }
}
